...
const getShareableLink = async (req, res) => {
  try {
    ...
    const _article = await article.findOne({ url });
    if (_article) {
      const id = _article._id;
      res.send({
        newsId: id
      });
    } else {
...
const getNewsById = async (req, res) => {
  try {
    ...
    const news = await article.findById(newsId);
    ...
    res.status(error.status || 500).json({
      success: false,
      error: "Failed to fetch news by ID",
      message: error.message,
    });
  }
};
